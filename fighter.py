...
class Fighter:
    def __init__(self, player, x, y, flip, data, sprite_sheet, animation_steps, sound):
        ...
        )  # all images of player
        self.action = 0  # 0:idle #1:run #2:jump #3:attack1 #4: attack2 #5:hit #6:death
        self.frame_index = 0
        self.image = self.animation_list[self.action][
            self.frame_index
        ]  # current image of player to be rendered
        self.update_time = pygame.time.get_ticks()
        self.rect = pygame.Rect(
            (x, y, 80, 180)
        )  # placeholder rect for handling all player movements
        self.vel_y = 0
        self.running = False
        self.jump = False
            ...
            for x in range(animation):
                ...
                temp_img_list.append(
                    pygame.transform.scale(
                        temp_img,
                        (self.width * self.image_scale, self.height * self.image_scale),
                    )
                )
    ...
    def move(self, screen_width, screen_height, target, round_over):
        ...
        if self.attacking == False and self.alive == True and round_over == False:
            ...
            if self.player == 1:
                ...
                if key[pygame.K_f] and self.supuse == 0 and self.health <= 50:
                    self.health = self.health * 1.8
                    self.health_boost = True
                    self.health_boost_endtime = (
                        pygame.time.get_ticks() + self.health_boost_duration
                    )
                    self.supuse = 1
                if key[pygame.K_x] and self.supuse == 0:
                    self.invincible = True
                    self.invincible_end_time = (
                        pygame.time.get_ticks() + self.invincible_duration
                    )
                    self.supuse = 1
            ...
            if self.player == 2:
                ...
                if key[pygame.K_KP1] or key[pygame.K_KP2]:
                    ...
                    if key[pygame.K_KP2]:
                        self.attack_type = 2
                    self.attack(target, self.attack_type)
                if key[pygame.K_KP3] and self.supuse == 0 and self.health <= 50:
                    self.health = self.health * 1.8
                    self.health_boost = True
                    self.health_boost_endtime = (
                        pygame.time.get_ticks() + self.health_boost_duration
                    ...
                    self.supuse = 1
                if key[pygame.K_KP4] and self.supuse == 0:
                    self.invincible = True
                    self.invincible_end_time = (
                        pygame.time.get_ticks() + self.invincible_duration
                    )
    ...
    def update(self):
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            # if the player is dead then end the animation else restart it
            if self.alive == False:
    ...
    def draw(self, surface):
        ...
        if self.invincible:
            circle_radius = (
                max(self.rect.width, self.rect.height) * 0.6
            )  # Adjust multiplier as needed    # Calculate center of the circle
            circle_center = (self.rect.centerx, self.rect.centery)
            pygame.draw.circle(
                surface, (255, 165, 0), circle_center, int(circle_radius), 2
            )
    ...
    def draw_timer(self, surface, x, y, radius):
        ...
        if percentage_completed >= 1:
            ...
            )  # Adjust the division factor to control pulsation speed
            # Adjust the alpha value range to [100, 255] for visibility
            brightness = abs(
                math.sin(pygame.time.get_ticks() / 2000)
            )  # Adjust the division factor to control pulsation speed
            ...
            glowing_yellow = (
                ...
                0,
                brightness,
            )  # Set yellow color with pulsating brightness
